...
export default function HealthDashboard() {
  ...
  records.forEach((r) => {
    if (r.vaccinations?.length > 0) {
      r.vaccinations.forEach((v) => allVaccinations.push(v));
    }
  });
  const overdueVax = allVaccinations.filter((v) => v.status === "Overdue" || v.status === "Vaccination Due");

  const nextAppointment = appointments.find((a) => a.status === "Upcoming" || a.status === "Scheduled") || null;
  ...
  const quickLinks = [
    ...
    { label: "Documents", path: "/health/documents", icon: FolderOpen, color: "text-violet-500 bg-violet-50" },
    { label: "Timeline", path: "/health/timeline", icon: GitBranch, color: "text-slate-500 bg-slate-50" },
    ...
    >
      <div className="space-y-8">
            ...
            {pets.map((p) => {
              ...
              return (
                <button
                  ...
                  className={`
                    flex items-center gap-3 rounded-full border px-4 py-2 text-sm font-black transition-all shrink-0
                    ${isSelected
            ...
            {quickLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.path}
                  to={link.path}
                  className="flex items-center gap-1.5 rounded-2xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-bold text-slate-600 transition hover:-translate-y-0.5 hover:shadow-sm hover:border-slate-300"
                >
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <StatisticCard
            label="Health Score"
            value={selectedPet.healthScore || latestRecord?.healthScore || 90}
            unit="%"
            trend="up"
          ...
          <StatisticCard
            label="Vaccines Due"
            value={overdueVax.length}
            trend={overdueVax.length > 0 ? "up" : "flat"}
            trendValue={overdueVax.length > 0 ? "Action needed" : "All current"}
            icon={Activity}
            iconColor={overdueVax.length > 0 ? "text-amber-600" : "text-teal-600"}
            iconBg={overdueVax.length > 0 ? "bg-amber-50" : "bg-teal-50"}
          />
          <StatisticCard
        ...
        {/* AI Summary Placeholder — Banner */}
        <AISummaryPlaceholder
          title="AI Health Summary"
          description={`PetVerse AI will analyze ${selectedPet.name}'s complete medical history, identify risk patterns, flag overdue care, and generate a proactive wellness plan.`}
          variant="banner"
        ...
        <div className="grid gap-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7 space-y-6">
            {/* Wellness Ring */}
            <HealthScoreCard score={selectedPet.healthScore || latestRecord?.healthScore || 90} petName={selectedPet.name} />
            ...
            <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              {latestRecord?.weight && latestRecord?.visitDate && (
                <p className="mt-2 text-[11px] font-semibold text-slate-400">
                  Last recorded: {new Date(latestRecord.visitDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                </p>
              )}
            </div>
            ...
            <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Link to="/health/medications" className="text-xs font-bold text-emerald-600 flex items-center gap-1">
              ...
              {activeMedications.length > 0 ? (
                ...
                <div className="bg-slate-50 border border-slate-100 p-4 text-center rounded-2xl">
                  <Pill className="mx-auto mb-2 text-slate-300" size={22} />
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm space-y-5">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Link to="/health/timeline" className="text-[10px] font-bold text-slate-400 hover:text-slate-600 flex items-center gap-0.5">
            ...
            <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Link to="/health/documents" className="text-xs font-bold text-emerald-600 flex items-center gap-1">
                  <span>All files</span><ChevronRight size={13} />
                </Link>
              </div>
